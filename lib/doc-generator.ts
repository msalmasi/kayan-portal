...
export async function generateSignedPdf(
  ...
  const green = rgb(0.1, 0.24, 0.16); // Kayan green

  let y = 720;

  // ── Header ──
  page.drawText("CERTIFICATE OF EXECUTION", {
    x: 140, y, size: 18, font: bold, color: green,
  });
  y -= 12;
  page.drawLine({
  ...
  page.drawText("Kayan International Inc.", {
  ...
  page.drawText("ELECTRONIC SIGNATURE", {
    x: 50, y, size: 12, font: bold, color: green,
  });
  y -= 24;
  ...
  const attestation = [
    ...
    "electronically signed the referenced SAFT Agreement through the Kayan",
    "Token Investor Portal. The signature was captured with the timestamp,",
  ...
  y -= 16;
  page.drawText(
    "Generated by Kayan Token Investor Portal • Confidential",
